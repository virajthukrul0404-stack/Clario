...
function Ensure-ArtifactRepository {
  ...
  try { $null = & $gcloud @describeArgs 2>&1 } catch {}
...
try {
  ...
  Write-Host "Granting public access to web service..."
  Invoke-Gcloud @(
    "run", "services", "add-iam-policy-binding", $WebServiceName,
    "--member=allUsers",
  ...
  if (-not $SkipSocket) {
    Write-Host "Granting public access to socket service..."
    Invoke-Gcloud @(
      "run", "services", "add-iam-policy-binding", $SocketServiceName,
      "--member=allUsers",
      "--role=roles/run.invoker",
      "--region", $Region,
      "--project", $ProjectId,
  ...
  if (-not $SkipSocket) {
    Write-Host "  Socket URL : $socketUrl" -ForegroundColor White
  }
